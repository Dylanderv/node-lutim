...
describe('Delete image', function () {
  ...
  it("should pass with mocked response", () => {
    return lutim.deleteImage('rJB4kl45', 'o0wbZGu1PbjifG076GjbTLMf')
      .then((res) => {
  ...
  it("should not be able to delete a not found image", () => {
    return lutim.deleteImage('not', 'existant')
      .then((res) => {
        expect(res).instanceOf(Object);
  ...
  it("should not pass without realShort", () => {
    return lutim.deleteImage(null, 'token')
      .then(() =>{
        Promise.reject(new Error('Expected method to reject'));
      })
      .catch((err) => {
  ...
  it("should not pass without token", () => {
    return lutim.deleteImage('realshort', null)
      .then(() =>{
        Promise.reject(new Error('Expected method to reject'));
      })
      .catch((err) => {
