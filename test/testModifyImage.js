...
describe('Modify image', function () {
  ...
  it("should pass with mocked response", () => {
    return lutim.modifyImage('rJB4kl45', 'o0wbZGu1PbjifG076GjbTLMf', 0, 0)
      .then((res) => {
  ...
  it("should pass without firstView parameter", () => {
    return lutim.deleteImage('rJB4kl45', 'o0wbZGu1PbjifG076GjbTLMf', 0)
      .then((res) => {
        expect(res).instanceOf(Object);
        expect(res.success).equal(true);
      });
  ...
  it("should not be able to modify a not found image", () => {
    return lutim.modifyImage('not', 'existant', 0, 0)
      .then((res) => {
        expect(res).instanceOf(Object);
  ...
  it("should not pass without realShort", () => {
    return lutim.modifyImage(null, 'token', 'deleteDay')
      .then(() =>{
        Promise.reject(new Error('Expected method to reject'));
      })
      .catch((err) => {
  ...
  it("should not pass without token", () => {
    return lutim.modifyImage('realShort', null, 'deleteDay')
      .then(() =>{
        Promise.reject(new Error('Expected method to reject'));
      })
      .catch((err) => {
  ...
  it("should not pass without deleteDay", () => {
    return lutim.modifyImage('realShort', 'deleteDay', null)
      .then(() =>{
        Promise.reject(new Error('Expected method to reject'));
      })
      .catch((err) => {
